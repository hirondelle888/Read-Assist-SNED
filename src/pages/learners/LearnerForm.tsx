      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            ...
            <Field
              label="Anonymized Identifier"
              htmlFor="anonymizedId"
              helper="Required when privacy mode is anonymized. This label can appear in dashboards and reports."
            >
              <Input
                id="anonymizedId"
                name="anonymizedId"
            ...
            <Field
              label="Data Access Sensitivity"
              htmlFor="dataAccessSensitivity"
              helper="Use restricted levels when external reports or medical/developmental details must be handled carefully."
            >
              <select
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field
                label="Disability Category or Identified Learning Need"
                htmlFor="disabilityCategory"
                helper="Required. This should reflect the learner support profile even when formal diagnosis is not yet available."
              >
                <Input
              ...
              <Field label="Attention / Behavior Support Level" htmlFor="attentionBehaviorSupportLevel">
                <select
                  id="attentionBehaviorSupportLevel"
                  name="attentionBehaviorSupportLevel"
                  value={formData.attentionBehaviorSupportLevel}
                  onChange={handleChange}
                  disabled={isSaved}
                  className={selectClassName}
                >
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>These fields summarize existing records when consent and authorization allow them to be used for educational support planning.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2">
            <Field label="Medical History Summary" htmlFor="medicalHistory">
              <Textarea id="medicalHistory" name="medicalHistory" value={formData.medicalHistory} onChange={handleChange} className="min-h-[110px]" disabled={isSaved} />
            </Field>
            <Field label="Developmental History Summary" htmlFor="developmentalHistory">
              <Textarea id="developmentalHistory" name="developmentalHistory" value={formData.developmentalHistory} onChange={handleChange} className="min-h-[110px]" disabled={isSaved} />
            </Field>
            <Field label="Family History Summary" htmlFor="familyHistory">
              <Textarea id="familyHistory" name="familyHistory" value={formData.familyHistory} onChange={handleChange} className="min-h-[110px]" disabled={isSaved} />
            </Field>
            <Field label="Academic History Summary" htmlFor="academicHistory">
              <Textarea id="academicHistory" name="academicHistory" value={formData.academicHistory} onChange={handleChange} className="min-h-[110px]" disabled={isSaved} />
            </Field>
            <Field label="OT / ST / ABA / SPED Report Summary" htmlFor="relatedServiceHistory" helper="Use this only for authorized educational summaries, not full confidential report reproduction.">
              <Textarea
                id="relatedServiceHistory"
                name="relatedServiceHistory"
                value={formData.relatedServiceHistory}
                onChange={handleChange}
        ...
        <Card>
          <CardHeader className="pb-4">
            ...
            <CardDescription>The system saves the learner support profile even when formal diagnosis details are absent, pending, or undisclosed.</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-wrap gap-2">
            ...
            <Badge variant="outline">Restricted data can be clearly labeled</Badge>
          </CardContent>
...
function parseForm(formData: FormState, interventionHistory: Learner["interventionHistory"]): Omit<Learner, "id" | "createdAt" | "updatedAt"> {
  return {
    ...
    historySummary: {
      ...
      developmentalHistory: formData.developmentalHistory.trim(),
      familyHistory: formData.familyHistory.trim(),
      academicHistory: formData.academicHistory.trim(),
      relatedServiceHistory: formData.relatedServiceHistory.trim(),
    },
...
function mapLearnerToForm(learner: Learner): FormState {
  return {
    ...
    medicalHistory: learner.historySummary.medicalHistory,
    developmentalHistory: learner.historySummary.developmentalHistory,
    familyHistory: learner.historySummary.familyHistory,
    academicHistory: learner.historySummary.academicHistory,
    relatedServiceHistory: learner.historySummary.relatedServiceHistory,
...
function Field({
  ...
  return (
    <div className="space-y-2">
      ...
      {children}
      {helper && <p className="text-xs leading-relaxed text-slate-500 dark:text-slate-400">{helper}</p>}
    </div>
  );
}
